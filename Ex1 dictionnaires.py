...
avoir_nom = etudiant.get("nom")

print(f"Le nom de l'étudiant est : {avoir_nom}")
...
etudiant.update({'courriel':'112344@cegepmontpetit.ca'})
avoir_adresse_courriel = etudiant.get("courriel")
print(f"Voici le courriel de l'étudiant qui a été ajouté : {avoir_adresse_courriel}")
...
print(f"Voici le nouveau courriel de l'étudiant : {nouveau_courriel}")
...
enlever_age = etudiant.pop("age")

print(f" on a enlevé l'âge de l'étudiant, sa valeur était: {enlever_age}")
...
nbr_valeur = len(etudiant)

print(f"Nous avons maintenant {nbr_valeur} paires clés valeurs.")